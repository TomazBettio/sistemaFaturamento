 ...
 *
 * Descricao: Relatório de nulidades que será enviado por e-mail (eventualmente usado)
 *
 * Alteracoes;
 *
 */
    ...
    public function index() {
        ...
        $sql = "SELECT mc.codigovendedor, mc.sigla, mc.empresa, mp.codigoprocesso
                ...
                WHERE mc.tipo_cliente = 'C' AND mc.status_cliente = 'A'
                    AND TRIM(mp.paismarca) = 'BR' AND (mp.codigomotcancel = 0 OR mp.codigomotcancel IS NULL) AND mp.codigotipoprocesso = 4
                    AND (SELECT COUNT(*) FROM marpaandamento AS ma WHERE mp.pasta = ma.pasta AND ma.codstatusandamento = 3) = 0
                    AND (SELECT ma.obsandamento FROM marpaandamento AS ma WHERE mp.pasta = ma.pasta ORDER BY ma.dataandamento DESC, ma.seq DESC LIMIT 1) LIKE '%NULIDADE ADMINISTRATIVA A REQUERIMENTO%';";
        $rows = query2($sql);

        $html = "<h2><strong>Nulidades:</strong></h2>";
        ...
        $param['assunto'] = 'Relatório de Nulidades';
        // enviaEmail($param);
            return $style . $html;
