...
class relatoriotabeladepara
{
	...
	{
		$this->_tabela->addColuna(array('campo' => 'CDPCODCLASSE', 'etiqueta' => 'C&oacute;digo Classe DE', 'tipo' => 'T', 'width' =>  200, 'posicao' => 'E'));
		$this->_tabela->addColuna(array('campo' => 'CDPCODCLASSEPROC', 'etiqueta' => 'Para', 'tipo' => 'T', 'width' =>  200, 'posicao' => 'E'));
	...
	private function getDados()
	{
		...
		if (is_array($rows) && count($rows) > 0) {
			foreach ($rows as $row) {
				$temp = [];
				$temp['CDPCODIGO']        = $row['CDPCODIGO'];
				$temp['CDPUSU_INC']       = $row['CDPUSU_INC'];
				$temp['CDPUSU_ALT']       = $row['CDPUSU_ALT'];
				$temp['CDPCODCLASSE']     = $row['CDPCODCLASSE'];
				$temp['CDPSTATUS']        = $row['CDPSTATUS'];
				$temp['CDPCODCLASSEPROC'] = $row['CDPCODCLASSEPROC'];
				$temp['CDPDATA_INC']      = datas::dataMS2D($row['CDPDATA_INC']);
				$temp['CDPDATA_ALT']      = $row['CDPDATA_ALT'];
