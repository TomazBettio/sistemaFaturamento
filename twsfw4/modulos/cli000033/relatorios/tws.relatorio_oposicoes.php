 ...
 *
 * Descricao: Relatório de oposições que será enviado por e-mail (eventualmente usado)
 *
 * Alteracoes;
 *
 */
...
class relatorio_oposicoes {
    ...
    public function index() {
        ...
        WHERE mc.tipo_cliente = 'C' AND mc.status_cliente = 'A'
          ...
          AND (SELECT TRIM(ma.obsandamento) FROM marpaandamento AS ma WHERE mp.pasta = ma.pasta ORDER BY ma.dataandamento DESC, ma.seq DESC LIMIT 1) LIKE 'OPOSI%O'";
        $rows = query2($sql);

        $html = "<h2><strong>Oposições:</strong></h2>";
        ...
        $param['assunto'] = 'Relatório de Oposicoes';
        // enviaEmail($param);
            // return $style . $html;
