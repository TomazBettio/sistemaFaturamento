...
/*
* Data Criação: 25/08/2014 - 10:16:29
* Autor: Thiel
*
* Descrição: Pedidos de compras liberados no dia (pela 215)
* 
* 
*  Alterções:
*
...
class pedido_liberados{
	...
	function __construct(){
		...
		$param['titulo']	= 'Relacao de Pedidos de Compras efetuados';
		...
		if(false){
			$help = 'Indica a data de liberação dos pedidos na rotina 215 do WinThor';
			ExecMethod('config.sys004.inclui',array('programa' => $programa, 'emp' => '', 'fil' => '', 'ordem' => '1', 'pergunta' => 'Data Liberação'	, 'variavel' => 'DATA'	, 'tipo' => 'D', 'tamanho' => '10', 'decimal' => 0, 'validador' => '', 'tabela' => '', 'funcaodados' => '', 'tipo_2' => '', 'help' => $help	, 'inicializador' => '', 'inicfunc' => '', 'opcoes' => ''));
			ExecMethod('config.sys004.inclui',array('programa' => $programa, 'emp' => '', 'fil' => '', 'ordem' => '2', 'pergunta' => 'Email'			, 'variavel' => 'EMAIL'	, 'tipo' => 'T', 'tamanho' => '60', 'decimal' => 0, 'validador' => '', 'tabela' => '', 'funcaodados' => '', 'tipo_2' => '', 'help' => ''	, 'inicializador' => '', 'inicfunc' => '', 'opcoes' => ''));
		}
	}			
	...
	function schedule($param,$data=''){
		...
		$emails = $param;
		if(count($dados) > 0){
			$tab = new tabela_gmail01(18);
			if($this->_teste){
				$tab->setTeste();
			}
			$tab->abreTabela(1000);
			$tab->addTitulo('Relacao de Pedidos de Compras liberados em '.$data);
			foreach ($dados as $dado){
				...
				if(!empty($dado['obs'])){
					$tab->abreTR();
						$tab->abreTD($dado['obs'],18,'esquerda');
					$tab->fechaTR();
				}
			...
			$tab->termos();
			if(!$this->_teste){
			    enviaEmail($emails, 'Pedidos de Compras. Data: '.$data,$tab);
			    log::gravaLog('PedidosCompras', 'Enviado email para: '.$emails);
			}
			else{
echo "".$tab;
			    enviaEmail('suporte@thielws.com.br', 'Pedidos de Compras. Data: '.$data,$tab);
			}
		}
...
//print_r($rows);
		if(count($rows) > 0){
			...
			foreach($rows as $row){
				...
				$ret[$i]['ultima'] 		= $this->getUltimaEntrada($row[0]);
...
//echo "$sql <br>\n";			
	...
	function getPedidos($data, $somente_dia = false){
		...
		$sql = "SELECT 
				...
				WHERE 
					p.DTLIBERA $simbolo to_date('$data','DD/MM/YYYY')
