...
* 
* Alteração:
* 			11/03/19 - Retirada restrição do codmarca <> 669 - Gustavo
...
class tabela24_sc{
	...
	function schedule($param){
		...
		log::gravaLog("precosAngeloni", 'Cria classe integratxt SC');
		$this->_integra = new integratxt01('', 1);
		log::gravaLog("precosAngeloni", 'Seta estrutura SC');
		$this->setEstruturaInc();
		
		log::gravaLog("precosAngeloni", 'Conecta FTP SC');
		...
		if(!$conn_id){
			log::gravaLog("precosAngeloni", "Nao foi possivel conectar ao servidor FTP SC".$servidor);
			die("Nao foi possivel conectar a $servidor");
		}
		$login_result = ftp_login($conn_id, $usuario, $senha);
		log::gravaLog("precosAngeloni", 'Limpa FTP SC');
		$this->limpaFTP($conn_id);
		
		$dados = array();
		log::gravaLog("precosAngeloni", 'Recupera dados SC');
		$dados = $this->getDados('SC');
		log::gravaLog("precosAngeloni", 'Gera Arquivo. Quant dados SC: '.count($dados));
		$link = $this->geraArquivo($dados, $data, $data, 'F','SC');
		log::gravaLog("precosAngeloni", 'Pega arquivo SC');
		$fpSC = fopen($this->_path.$this->_arq, 'r');
		
		log::gravaLog("precosAngeloni", 'Transfere para ftp SC');
		if (ftp_fput($conn_id, $this->_arq, $fpSC, FTP_ASCII)) {
			log::gravaLog("precosAngeloni", "Enviado SC com sucesso arquivo para o FTP SC: ".$this->_arq);
		} else {
			log::gravaLog("precosAngeloni", "Erro SC ao enviar arquivo para o FTP SC: ".$this->_arq);
		}
		
		fclose($fpSC);
		log::gravaLog("precosAngeloni", 'Copia BKP SC');
		copy($this->_path.$this->_arq, $dir.$this->_arq.'_'.$data.'.txt');
		//O arquivo já é gerado no temp, então não precisa copiar
		//log::gravaLog("precosAngeloni", 'Copia Temp SC');
	...
	function geraArquivo($dados, $dtDe, $dtAte, $tipo,$uf){
		global $config;
		if($this->_log){
			log::gravaLog("precosAngeloni", "Executando geraArquivo SC: $dtDe, $dtAte, $tipo,$uf");
		}
...
//print_r($dados);
		...
		log::gravaLog("precosAngeloni", 'Seta diretorio saida SC');
		$this->_integra->setDiretorios(array('saida' => $config['temp']));
		
		$this->_path = $config['temp'];
		$this->_arq = $arquivo;
		
		log::gravaLog("precosAngeloni", 'Gera arquivo tipo 0 SC');
		...
		if($this->_log){
			log::gravaLog("precosAngeloni", "Gerando tipo 0 SC $arquivo");
		}
		
		log::gravaLog("precosAngeloni", 'Prepara dados SC. Quant: '.count($dados));
		//Ajusta os precos (multiplica por 100 para ficar
		$i = 0;
		$ret = array();
		foreach($dados as $dado){
			$preco = getPrecoFabrica($dado['cod'],'SC');
			$ret[$i]['ean'] 		= $dado['ean'];
...
//echo "Cod: ".$dado['cod']."  Preco: ".$ret[$i]['preco'] ."  Desconto: ".$ret[$i]['desconto']." ".$dado['precoTabela'] ."<br>\n";
			if($ret[$i]['desconto'] < 0){
				$ret[$i]['desconto'] = 0;
		...
		$quant = count($ret);
		log::gravaLog("precosAngeloni", 'Dados Processados SC: '.$quant);
		log::gravaLog("precosAngeloni", 'Gera tipo 2 SC');
		$this->_integra->gravaArquivoTipo3(2, $arquivo, $ret);
		if($this->_log){
			log::gravaLog("precosAngeloni", "Gerando tipo 2 SC $arquivo");
		}
		
		log::gravaLog("precosAngeloni", 'Gera tipo 3 SC');
		$this->_integra->gravaArquivoTipo3(3, $arquivo, array(array('quant'=>$quant)));
		if($this->_log){
			log::gravaLog("precosAngeloni", "Gerando tipo 3 SC $arquivo");
		}
...
//print_r($ret);	
	...
	function getPreco($cod,$uf){
		...
		/*
		 * Alterado para pegar o preço fábrica na tabela PCTABMEDABCFARMA
		 */
	
		$sql = "SELECT PRECOFABRICA ";
