...
if (! defined('TWSiNet') || ! TWSiNet)
	...
	class cad_organizacoes{
		...
		function index(){
			...
			$bw->addColuna(array('campo' => 'tipo' 	    , 'etiqueta' => 'Tipo'		    , 'tipo' => 'T', 'width' =>  10, 'posicao' => 'E'));
			...
			$param = [];
			$param['titulo'] = 'Cadastro de Organizações';
			$param['conteudo'] = $bw;
		...
		private function getDados(){
			$ret = array();
			$col = array('id','nome','nreduz','ie','resp','cnpj','cod','endereco','bairro','cidade','uf','cep','contato','fone','hpage','diafat', 'tipo');
			
			$rows = query("SELECT * FROM cad_organizacoes WHERE 1=1");
			if(is_array($rows) && count($rows)>0){
			    $dicionario_tipos = montarDicionarioSys005('CRM016');
				foreach ($rows as $r) {
				    foreach ($col as $c){
						$temp[$c] = $r[$c];
				    }
				    if(!empty($temp['tipo'])){
				        $temp['tipo'] = $dicionario_tipos[$temp['tipo']];
				    }
					$ret[] = $temp;
				}
			}
		...
		private function getDados2($id){
			...
			if($id != 0){
				$rows = query("SELECT * FROM cad_organizacoes WHERE id = $id ");
				if(is_array($rows) && count($rows)>0){
					foreach ($rows as $r) {
						foreach ($col as $c){
							$ret[$c] = $r[$c];
		...
		public function editar(){
			...
			$form->addCampo(array('id' => '','campo' => 'formPrograma[tipo]',      'etiqueta' => 'Tipo',               'tipo' => 'A','tamanho' => '60','linhas' => '','valor' => $dados['tipo'],       'pasta' => 0, 'lista' => tabela('CRM016'),'validacao' => '','largura' => 4,'obrigatorio' => true));
		...
		public function salvar(){
			...
			if(isset($_POST['formPrograma'])&&count($_POST['formPrograma'])>0){
				...
				if($id==0){
					...
					$dados['emp'] = '';
					$dados['fil'] = '01';
					$sql = montaSQL($dados, 'cad_organizacoes');
					query($sql);
				}
				else {
					$sql = montaSQL($dados, 'cad_organizacoes', 'UPDATE', "id = $id ");
					query($sql);
				}
			}
			return $this->index();
		...
		{
			$id = getParam($_GET, 'id', 0);
			query("UPDATE cad_organizacoes SET ativo = 'N' WHERE id = $id ");
			return $this->index();
		}
		
		private function getNovoCliente(){
			$ret = '000001';
			$sql = "select max(cod) as cod from cad_organizacoes where cod < '900000'";
			$rows = query($sql);
			if(is_array($rows) && count($rows) > 0){
				$temp = $rows[0]['cod'];
				$ret = intval($temp) + 1;
