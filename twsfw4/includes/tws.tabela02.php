 ...
 *
 * Altera��es:
 * 				28/06/23 - Thiel - Inclusão de filtro
 *
 */
...
class tabela02{
	...
	private $_filtroTab;		//Indica se vai utilizar filtro
	...
	private $_mostraFiltro;
	
	//Indica se é a promeira vez eu a tabela está sendo executada (controle por sessao)
	private $_appVarFiltro;
	
	public function __construct($param = []){
		$this->carregaScripts();
		$this->_programa = $param['programa'] ?? getModulo().'_'.getClasse();
		...
		$this->_mostraFiltro = $param['mostraFiltro'] ?? false;
		...
		$this->_appVarFiltro = getAppVar($this->_programa.'_filtro');
		if($this->_appVarFiltro == null){
			$this->_appVarFiltro = false;
		}
		...
		if($this->_mostraFiltro){
			$paramFiltro = [];
			if(isset($param['linkFiltro'])){
				$paramFiltro['link'] = $param['linkFiltro'];
			}
			$paramFiltro['colunas'] = $param['colunasFiltro'] ?? 2;
		...
		$this->_filtroTab	= $param['filtro'] ?? true;
	...
	public function __toString(){
		...
		// Se não é a primeira vez sendo executado esconde o filtro
	...
	/**
	 * Imprime ultima linha da tabela
	 * TODO: implementar
	 */
	...
	/**
	 * Retorna as chave/valor dos filtros
	 * 
	 * @return array chave/valor filtros
	 */
	function getFiltro(){
