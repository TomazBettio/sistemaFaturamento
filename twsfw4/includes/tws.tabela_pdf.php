...
class tabela_pdf{
	...
	private $_stripeCor1 = '#FFFFFF';
	private $_stripeCor2 = '#efefef';
	...
	function setTabela($campos, $cab, $width = array(), $posicao = array(), $tipo = array()){
		...
		foreach ($campos as $i => $campo){
			...
			$temp['pos'] 	= isset($posicao[$i]) ? $posicao[$i] : 'E';
...
//print_r($this->_colunas);
	...
	public function addColuna($param){
		...
		$temp['pos'] 	= isset($param['posicao']) && !empty($param['posicao']) ? $param['posicao'] : 'E';
	...
	private function ajustaTamanhoPercentual(){
	...
	private function getLinhas($total = false){
		global $nl;
		$ret = '';
		$dados = array();
