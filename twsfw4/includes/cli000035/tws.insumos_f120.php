...
class insumos_F120{
	...
	public function __construct($path, $anos, $tipo, $trace = false){
		...
		$this->_bloco = 'F120';
		$this->_blocosAtivos = ['F120'];
		
		$this->_resumo = true;
		$this->_arqResumo = 'F120_resumo';
...
// ----------------------------------------INI Processo especifico ------------------------------------------------------------------
						...
						//if($sep[2] == '0'){
							...
							$pis = str_replace(',', '.', $sep[11]);
							$cofins = str_replace(',', '.', $sep[15]);
							$seq = $sep[16]; # Conta Contábil
							$produto = $sep[18];
							
							$ret[] = ['data'=>$data, 'nota'=>$nota, 'vl'=>$valor, 'pis'=>$pis, 'cof'=>$cofins, 'bloco' =>'F120', 'seq'=>$seq, 'prod'=>$produto];
						//}
