...
class insumos_f100{
	...
	public function __construct($path, $anos, $tipo, $trace = false){
		...
		$this->_bloco = 'F100';
		$this->_blocosAtivos = ['F100'];
		
		$this->_resumo = true;
		$this->_arqResumo = 'F100_resumo';
...
// ----------------------------------------INI Processo especifico ------------------------------------------------------------------
						//Sujeita à Incidência de Crédito de PIS/Pasep ou Cofins
						if($sep[2] == '0'){
							$nota = '00000';#$sep[8];
							$data = $sep[5];
							$valor = str_replace(',', '.', $sep[6]);
							$pis = str_replace(',', '.', $sep[10]);
							$cofins = str_replace(',', '.', $sep[14]);
							$seq = $sep[17]; # Conta Contábil
							$produto = $sep[3];
							
							$ret[] = ['data'=>$data, 'nota'=>$nota, 'vl'=>$valor, 'pis'=>$pis, 'cof'=>$cofins, 'bloco' =>'F100', 'seq'=>$seq, 'prod'=>$produto];
						}
