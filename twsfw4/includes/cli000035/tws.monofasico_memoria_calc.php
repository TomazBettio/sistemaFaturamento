...
class monofasico_memoria_calc {
    ...
    public function index() {
		...
		$dados = $this->getDadosRelatorio($this->_cnpj);
	...
	private function getDadosRelatorio($cnpj) {
        $ret = [];

        $arquivos = $this->getArquivos($this->_path . $cnpj . DIRECTORY_SEPARATOR . 'arquivosExcel');

		$pis_per_comp = [];

        foreach ($arquivos as $arquivo) {
			if(strpos($arquivo, 'C400') !== false || strpos($arquivo, 'C100') !== false || strpos($arquivo, 'C860') !== false){ 
				$this->lerGeral($cnpj, $arquivo);
			} else if(strpos($arquivo, 'M600') !== false) {
				$pis_per_comp = $this->lerM600($cnpj, $arquivo);
			}
        }
	...
	private function lerGeral($cnpj, $arquivo) {
		$files = glob($this->_path . $cnpj . DIRECTORY_SEPARATOR . 'arquivosExcel' . DIRECTORY_SEPARATOR . $arquivo);
				...
				if(!empty($linha)) {
					...
					if(strpos($filename, 'C400') !== false) {
						if($sep[16] == 01) { // só roda se o CST for 01
							...
							$dados = [];
							$dados['periodo'] = $sep[1];
							$dados['base_calc_pis'] = str_replace('.', '', $sep[18]); // esse
                            $dados['base_calc_pis'] = str_replace(',', '.', $dados['base_calc_pis']);
                            $dados['base_calc_pis'] = str_replace(' ', '', $dados['base_calc_pis']);
							$C400[] = $dados;
	...
	private function lerM600($cnpj, $arquivo) {
		$ret = [];
		$files = glob($this->_path . $cnpj . DIRECTORY_SEPARATOR . 'arquivosExcel' . DIRECTORY_SEPARATOR . $arquivo);
