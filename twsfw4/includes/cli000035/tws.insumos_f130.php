...
class insumos_F130{
	...
	public function __construct($path, $anos, $tipo, $trace = false){
		...
		$this->_bloco = 'F130';
		$this->_blocosAtivos = ['F130'];
		
		$this->_resumo = true;
		$this->_arqResumo = 'F130_resumo';
...
// ----------------------------------------INI Processo especifico ------------------------------------------------------------------
						...
						if($sep[2] == '0'){
							...
							$pis = str_replace(',', '.', $sep[14]);
							$cofins = str_replace(',', '.', $sep[18]);
							$seq = $sep[19]; # Conta Contábil
							$produto = $sep[21];
							
							$ret[] = ['data'=>$data, 'nota'=>$nota, 'vl'=>$valor, 'pis'=>$pis, 'cof'=>$cofins, 'bloco' =>'F130', 'seq'=>$seq, 'prod'=>$produto];
						}
