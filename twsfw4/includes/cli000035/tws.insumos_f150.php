...
class insumos_F150{
	...
	public function __construct($path, $anos, $tipo, $trace = false){
		...
		$this->_bloco = 'F150';
		$this->_blocosAtivos = ['F150'];
		
		$this->_resumo = true;
		$this->_arqResumo = 'F150_resumo';
...
// ----------------------------------------INI Processo especifico ------------------------------------------------------------------
						...
						if($sep[2] == '0'){
							$nota = '00000';#$sep[8];
							$data = ''; #$sep[10];
							$valor = str_replace(',', '.', $sep[5]);
							$pis = str_replace(',', '.', $sep[9]);
							$cofins = str_replace(',', '.', $sep[12]);
							$seq = $sep[14]; # Conta Contábil
							$produto = '';
							
							$ret[] = ['data'=>$data, 'nota'=>$nota, 'vl'=>$valor, 'pis'=>$pis, 'cof'=>$cofins, 'bloco' =>'F150', 'seq'=>$seq, 'prod'=>$produto];
						}
