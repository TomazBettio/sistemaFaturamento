...
if (!defined('TWSiNet') || !TWSiNet)
	...
	class monofasico_analise02
	{
		...
		public function __construct($cnpj, $contrato, $id)
		{
			...
			$param['titulo'] = 'Analise Simplificada - '.$this->_titulo;
		...
		public function index()
		{
			...
			$botao_processa["onclick"] = "setLocation('" . getLink() . "analise02.refazerAnalise&cnpj=" . $this->_cnpj . '|' . $this->_contrato . '|' . $this->_id  . "')";
			$this->_relatorio->addBotao($botao_processa);
			
			$ret .= $this->_relatorio;
			
			$param = [];
			$param['acao'] = getLink() . "analise02.salvarItens&cnpj=" . $this->_cnpj . '|' . $this->_contrato . '|' . $this->_id;
			$param['id'] = 'formItens';
			...
			if (count($itens) > 0) {
				...
				if (is_file($arquivo)) {
					...
					foreach ($dados as $key => $dado) {
						$id = $dado['cnpj_forn'].'-'.$dado['cod_item'].'-'.$dado['ncm'].'-'.$dado['cfop'];
						if (isset($itens[$id])) {
		...
		private function recuperaArquivo()
		{
			...
			foreach ($arquivo as $linha) {
				...
				if ($temp['selecionado'] == 'S') {
					$cnpj_fornec= $temp['cnpj_forn'];
					$produto 	= $temp['cod_item'];
					$ncm 		=  $temp['ncm'];
					$cfop 		=  $temp['cfop'];
					
					if(!isset($dados[$cnpj_fornec][$produto][$ncm][$cfop])){
						$dados[$cnpj_fornec][$produto][$ncm][$cfop] = $temp;
					}else{
						$dados[$cnpj_fornec][$produto][$ncm][$cfop]['vl_item'] 				+= $temp['vl_item'];
						$dados[$cnpj_fornec][$produto][$ncm][$cfop]['vl_desc'] 				+= $temp['vl_desc'];
						$dados[$cnpj_fornec][$produto][$ncm][$cfop]['vl_base'] 				+= $temp['vl_base'];
						$dados[$cnpj_fornec][$produto][$ncm][$cfop]['vl_final_pis'] 		+= $temp['vl_final_pis'];
						$dados[$cnpj_fornec][$produto][$ncm][$cfop]['vl_final_cofins'] 		+= $temp['vl_final_cofins'];
						$dados[$cnpj_fornec][$produto][$ncm][$cfop]['vl_calc_final_pis'] 	+= $temp['vl_calc_final_pis'];
						$dados[$cnpj_fornec][$produto][$ncm][$cfop]['vl_calc_final_cofins'] += $temp['vl_calc_final_cofins'];
			...
			if(count($dados) > 0){
				foreach ($dados as $forn => $d1){
					foreach ($d1 as $produto => $d2){
						foreach ($d2 as $ncm => $d3){
							foreach ($d3 as $cfop => $d){
								$checked = '';
								$d['sel'] =  '<input name="item['.$forn.'-'.$produto.'-'.$ncm.'-'.$cfop.']" type="checkbox" value="" ' . $checked . ' id="'.$forn.'-'.$produto.'-'.$ncm.'-'.$cfop.'">';
								$ret[] = $d;
							}
						}
					}
