...
if(true){
	...
	// Conectar base teste
	$config["db4_banco"] 	= "oci8";
	$config["db4_server"] 	= "10.0.0.93";
	$config["db4_database"] = "TESTE";
	$config["db4_usuario"] 	= "gauchafarma";
	$config["db4_senha"] 	= "GAUCHATESTE";
...
function getCampoSupervisor($super, $campo = 'nome'){
	$ret = '';
	if($campo == 'email'){
		$campo = "(select us2.email2 from pcusuari us2 where us2.codusur = pcsuperv.cod_cadrca) email";
	}
...
function getDadosRCA($rca){
	...
	$sql = "select pcusuari.codusur,
						       ...
						       (select us2.email from pcusuari us2 where us2.codusur = pcsuperv.cod_cadrca) EMAIL_SUPER,
								EMAIL2,
							   (select us2.email2 from pcusuari us2 where us2.codusur = pcsuperv.cod_cadrca) EMAIL_SUPER2
						from pcusuari,
						     pcsuperv
	...
	foreach ($rows as $row){
		$ret['rca'] = $row[0];
		$ret['nome'] = $row[1];
		$ret['email'] = !empty(trim($row['EMAIL2'])) ? trim($row['EMAIL2']) : $row['EMAIL'];
		$ret['super'] = $row[3];
		$ret['super_nome'] = $row[4];
		$ret['super_email'] = !empty(trim($row['EMAIL_SUPER2'])) ? trim($row['EMAIL_SUPER2']) : $row['EMAIL_SUPER'];
	}
	return $ret;
}
...
function getEmailCoordenador($super){
	...
	$sql = "select pcsuperv.codsupervisor,
							   ...
							   (select us2.email2 from pcusuari us2 where us2.codusur = pcsuperv.cod_cadrca) EMAIL2
						from pcsuperv
	...
	if(count($rows) > 0){
		//$ret = $rows[0][2];
		$ret = !empty(trim($rows[0]['EMAIL2'])) ? trim($rows[0]['EMAIL2']) : $rows[0]['EMAIL'];
	}
...
function getEmailERC($erc){
	$ret = '';
	$sql = "select PCUSUARI.EMAIL, PCUSUARI.EMAIL2 from pcusuari where pcusuari.codusur = $erc";
	//echo "SQL: $sql <br> \n";
	$rows = query4($sql);
	if(count($rows) > 0){
		//$ret = $rows[0][0];
...
function getListaEmailGF($tipo,$ativo = true,$ordem = '', $filtrar = true, $indice = ''){
	$ret = array();
	switch ($tipo) {
		case 'rca':
		case 'erc':
			$naoEnviarEmail = $filtrar ? array(11,268,600) : array();
			$sql = "SELECT PCUSUARI.CODUSUR,
						       PCUSUARI.NOME,
						       PCUSUARI.EMAIL,
						       PCUSUARI.CODSUPERVISOR,
			...
			foreach ($rows as $row){
				...
				$direto = false;
				if(strpos(strtoupper($ret[$i]['nome']), "DIRETO") !== false && $filtrar){
					$direto = true;
				}
				
				
				if(array_search($row[0], $naoEnviarEmail) === false && $ret[$i]['ativo'] == 'S' && !$direto){
					$ret[$i]['email'] = !empty(trim($row['EMAIL2'])) ? $row['EMAIL2'] : $row['EMAIL'];
				}else{
					$ret[$i]['email'] = '';
				}
				$ret[$i]['super'] = $row[3];
				$ret[$i]['super_nome'] = $row[4];
				$ret[$i]['super_email'] = !empty(trim($row['EMAIL_SUPER2'])) ? $row['EMAIL_SUPER2'] : $row['EMAIL_SUPER'];
				$i++;
			}
			break;
		case 'supervisor':
			...
			foreach ($rows as $row){
				...
				if(array_search($row[0], $naoEnviarEmail) === false){
					$ret[$i]['email'] = !empty(trim($row['EMAIL2'])) ? trim($row['EMAIL2']) : $row['EMAIL'];
				}else{
					$ret[$i]['email'] = '';
				}
				if($row[0] == 19){
...
function vendas1464Campo($campos,$dataIni, $dataFim, $param, $trace = false){
	...
	if($trace){
		echo "DataIni: $dataIni - DataFim: $dataFim <br>\n";
		print_r($campos);
		print_r($param);
	}
	...
	// vendedor = tanto no pedido ou no código de vendedor do cliente
	if(isset($param['vendedor']) && !empty($param['vendedor'])){
		$whereVENDEDOR = " AND (PCNFSAID.CODUSUR IN ( ".$param['vendedor']." ) OR PCNFSAID.CODCLI IN ( select codcli from pcclient where codusur1 in (".$param['vendedor'].")))";
		$whereVENDEDORdev = " AND (PCNFENT.CODUSURDEVOL IN ( ".$param['vendedor']." ) OR PCNFENT.CODFORNEC IN (  select codcli from pcclient where codusur1 in (".$param['vendedor'].")))";
	}
	...
	if(isset($param['rede']) && !empty($param['rede'])){
		$whereRede = " AND (PCNFSAID.CODCLI IN ( SELECT CODCLI FROM PCCLIENT WHERE CODREDE = ".$param['rede']." ))";
		$whereRedeDev =" AND (PCNFENT.CODFORNEC IN ( SELECT CODCLI FROM PCCLIENT WHERE CODREDE = ".$param['rede']." ))";
	}
...
function getListaMarcas(){
	...
	$sql = "SELECT CODMARCA, MARCA FROM PCMARCA WHERE ATIVO = 'S'";
	$rows = query4($sql);
...
function listaRecursos($ativo = true,$primeiroBranco = true){
	$usuarios = array();
	if ($ativo === true){
		$where = "ativo = 'S' ";
	}else{
	...
	$sql = "SELECT user, apelido FROM sys001 WHERE 1=1 AND ".$where." ORDER BY apelido";
	$rows = query($sql);
...
function getPrecoFabrica($cod,$uf){
	$ret = 0;
	
	$sql = "SELECT PRECOFABRICA FROM PCTABMEDABCFARMA WHERE CODPROD = $cod AND UF = '$uf'";
	$rows = query4($sql);
	
	if(isset($rows[0]['PRECOFABRICA'])){
		$ret = $rows[0]['PRECOFABRICA'];
	}
	
	return $ret;
}

function getTodosPrecosFabrica($uf){
	$ret = [];
	
	$sql = "SELECT CODPROD, PRECOFABRICA FROM PCTABMEDABCFARMA WHERE UF = '$uf'";
	$rows = query4($sql);
	
	if(is_array($rows) && count($rows) > 0){
		foreach ($rows as $row){
			$ret[$row['CODPROD']] = $row['PRECOFABRICA'];
		}
	}
